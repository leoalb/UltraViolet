...
class WorkerServer(object):
    ...
    def start(self, ip, port, password, retry=float("inf")):
        ...
        print("hostname - ", self.config.get_worker_ip(self.get_hostname()))
    ...
    def get_hostname(self):
        return str(subprocess.check_output(["hostname"]).decode(sys.stdout.encoding)).replace("\n","")
...
class MininetManager(object):
    ...
    @Pyro4.expose
    def fetchHostLinks(self, node, cls=None, **params):
        node = self.net.get(node)
        nodeintfs = {}
        for intf in node.intfList():
            if intf.link:
                nodeintfs[intf] = str(intf.link)
        return str(nodeintfs)

    @Pyro4.expose
    def updateLink(self, nodeintf, choice, val, **params):
        node = self.net.get(nodeintf[0])
        if choice == 1:         #bandwidth
            node.intfList()[nodeintf[1]].link.intf1.config(bw = int(val))
            node.intfList()[nodeintf[1]].link.intf2.config(bw = int(val))
        elif choice == 2:       #delay
            node.intfList()[nodeintf[1]].link.intf1.config(delay = str(val)+"ms")
            node.intfList()[nodeintf[1]].link.intf2.config(delay = str(val)+"ms")
        elif choice == 3:       #loss
            node.intfList()[nodeintf[1]].link.intf1.config(loss = int(val))
            node.intfList()[nodeintf[1]].link.intf2.config(loss = int(val))
        else:
            print("Incorrect Choice while choosing")
            return
