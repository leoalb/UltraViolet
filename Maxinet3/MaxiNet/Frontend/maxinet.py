...
class Worker(object):
    ...
    def fetchHostLinks(self, node, cls=None, **params):
        return self.mininet.fetchHostLinks(node)

    def updateLink(self, intfready, choice, val, **params):
        return self.mininet.updateLink(intfready, choice, val)
...
class Experiment(object):
    ...
    def fetchHostLinks(self, node):
        w = self.get_worker(node)
        if(not isinstance(node, NodeWrapper)):
            node = self.get(node)
        return w.fetchHostLinks(self.name(node))
    ...
    def updateLink(self, intfready, choice, val, **params):
        ...
        if(not isinstance(node, NodeWrapper)):
            node = self.get(node)
        w.updateLink(intfready, choice, val)
